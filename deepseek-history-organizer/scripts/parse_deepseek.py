...
def extract_file_entries(files):
    """把分享抓取产出的 message.files 转成附件元数据（新版接口附件在 FILE 片段里）。"""
    entries = []
    ...
    for f in files:
        ...
        url = f.get("signed_path") or f.get("url") or f.get("data") or ""
        ftype = "image" if f.get("is_image") else "file"
        if name or url:
            entries.append({"type": ftype, "name": name, "url": url})
    return entries
...
def normalize_message(item):
    ...
    attachments = extract_attachments(msg.get("content")) + extract_file_entries(msg.get("files"))
