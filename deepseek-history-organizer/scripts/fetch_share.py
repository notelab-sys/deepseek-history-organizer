...
# 2026-09 起 DeepSeek 分享接口改用 fragments 承载正文：
#   USER      消息 = FILE（附件）+ REQUEST（提问正文）
#   ASSISTANT 消息 = THINK（思考过程）+ RESPONSE（回答正文）
# 旧接口把正文直接放在 message.content，故两者都要兼容。
TEXT_FRAGMENT_TYPES = ("REQUEST", "RESPONSE")


def fragment_text(msg):
    """取消息正文：优先新版 fragments，回退旧版 content。"""
    parts = []
    for frag in msg.get("fragments") or []:
        if not isinstance(frag, dict):
            continue
        if str(frag.get("type", "")).upper() in TEXT_FRAGMENT_TYPES:
...
def fragment_files(msg):
    """取消息附件：新版在 FILE 片段里，旧版在 message.files。"""
    files = []
    for frag in msg.get("fragments") or []:
        if isinstance(frag, dict) and str(frag.get("type", "")).upper() == "FILE":
            for item in frag.get("files") or []:
                if isinstance(item, dict):
                    files.append(item)
    if not files and isinstance(msg.get("files"), list):
        files = [f for f in msg["files"] if isinstance(f, dict)]
    return files
...
def make_conversation(share, raw):
    ...
    title = None
    for m in msgs:
        text = fragment_text(m)
        if role_map(m.get("role")) == "user" and text.strip():
        ...
        "messages": [
            {
                ...
                "content": fragment_text(m),
                "create_time": m.get("inserted_at"),
                "files": fragment_files(m),
            }
