...
- **归纳文档（语言自适应）**：Markdown → Word / PDF，便于打印与传阅；中文用户默认中文期刊规范排版，英文用户以 `--lang en` 输出英文期刊格式（Times New Roman 12pt、加粗标题、APA 悬挂缩进、Page X of Y 页码）。
- **速览卡片与登记表**：每段对话一张速览卡，归档文件夹自动生成登记表。
- **多条对话综合分析**：每条对话独立、完整归纳；相近主题可合并、不同主题单独成文，主题相近自动提示，按时间连续编号归档。
- **分享链接批量抓取**：DeepSeek 下载页无法筛选指定对话时，按分享链接批量获取并合并。
- **本地关键词检索**：整包导出后本地检索筛选，命中对话可导出子集。
- **附件正文提取**：读取 Word/PDF/Excel 等附件文档正文（支持开放获取论文补全）。
- **参考文献核查（中英文）**：半自动核查英文文献（Crossref 解析 DOI / 书目检索 + PubMed 双通道，带 PMID），中文文献提示经知网 CNKI、万方、维普、期刊官网人工核对，输出"倾向"标记候选表，供人工复核后定稿。
...
已完成 9 段真实对话测试（场景覆盖：全流程、附件提取、文献核查、多段合并、新工具、去重、无文献、附件补全、组学分析选型）。测试内容已脱敏，仅以编号与场景类型列出，见 [测试记录](测试记录.md)。
